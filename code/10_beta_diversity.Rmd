...
To compare the `Diet` and `Compartment` effect, we need to run two-way PERMANOVA with 2 nested random effects (`FishID` and `Tank`). Unfortunately, this can't be done in R. We export the distance matrix and run PERMANOVA using the PERMANOVA+ add-on from the [PRIMER v7](https://www.primer-e.com/our-software/permanova-add-on/).
...
```{r}
filter(rpca_mcsFdWt, SampleOrigin %in% c("Mucosa", "Water")) %>% # get sample ids of mucosa and water samples
  pull(SampleID) %>%
...
pmanova_main1 <- pmanova_main %>%
  ...
  column_to_rownames("SampleOrigin") %>%
  mutate(across(everything(), as.numeric)) %>%
  rownames_to_column("SampleOrigin")
```
...
```{r}
pmanova <- full_join(pmanova_main1, pmanova_cc1, by = "SampleOrigin") %>%
  rename("Sample origin" = SampleOrigin) %>%
  gt() %>%
...
The PERMANOVA results suggested little evidence of `Tank` effect. Thus, we use individual fish for the testing of homogeneity of multivariate dispersion. As there's a significant interaction between the `Diet` and `Compartment` effect, we assess homogeneity of multivariate dispersion for each main effect stratified by the levels of the remaining main effect.
...
The PERMANOVA results suggested little evidence of `Compartment` effect. We only need to check the homogeneity of multivariate dispersion for the other main effect,`Diet`.

### Filter data

```{r}
# filter data to keep mucosa samples only
rpca_mcs <- filter(rpca_mcsFdWt, SampleOrigin == "Mucosa")
