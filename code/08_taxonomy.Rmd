...
taxa_bar_digesta <- filter(taxa_tab, SampleOrigin == "Digesta") %>%
  ...
  facet_nested(~ SampleOrigin + Diet + Compartment, scales = "free_x", nest_line = TRUE) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
...
coreHmp <- ggarrange(
  plotlist = coreHmp, nrow = 1, labels = c("OTUs", ""), label.y = 0.55, 
  font.label = list(size = 10, face = "plain"), legend = "right", common.legend = TRUE)

coreHmp

# export plot
ggsave(here("result/figure/Figure 2.tiff"), width = 8.5, height = 10,
       units = "in", dpi = 300, compression = "lzw")
```
...
ovrl <- bind_cols(prvl) %>%
  ...
  mutate(
    WaterOverlap = case_when(
      Water >  1 & Intestine >  1 ~ "Shared",
      Water >  1 & Intestine <= 1 ~ "Unique to water",
      Water <= 1 & Intestine >  1 ~ "Unique to intestine",
      TRUE ~ "Absent"),
    FeedOverlapREF = case_when(
      FeedREF == 1 & Intestine >  1 ~ "Shared",
      FeedREF == 1 & Intestine <= 1 ~ "Unique to feed",
      FeedREF == 0 & Intestine >  1 ~ "Unique to intestine",
      TRUE ~ "Absent"),
    FeedOverlapIM = case_when(
      FeedIM == 1 & Intestine >  1 ~ "Shared",
      FeedIM == 1 & Intestine <= 1 ~ "Unique to feed",
      FeedIM == 0 & Intestine >  1 ~ "Unique to intestine",
      TRUE ~ "Absent")) %>%
...
```{r}
shared_water <- prune_taxa(names(which(prvl[["Water"]] > 0)), ps_tss) %>% sample_sums()
...
bp_shared_feed <- ggplot(shared_feed, aes(x = Compartment, y = Abundance)) +
  ...
  labs(x = "Comparment", y = "Relative abundance (%)") +
...
# export plot
ggsave(here("result/figure/Figure 3.tiff"), width = 7, height = 8,
       units = "in", dpi = 300, compression = "lzw")
```
