...
library(webshot2) # Take Screenshots of Web Pages, [github::rstudio/webshot2] v0.0.0.9000
...
txnm <- tax_table(ps) %>% as("matrix") %>% as_tibble(rownames = NA)
...
plot_grid(
  taxa_bar_noLgnd, lgnd_gg, nrow =  1, align = 'h', rel_widths = c(4, 1)
  ) 
...
ovrl_bar_feed <- ovrl %>%
  ...
  scale_fill_manual(values = c("#66C2A5", "#B3B3B3", "#E5C494")) +
...
ovrl_bar_water <- ovrl %>% 
  ...
  labs(x = "", y = "Number of OTUs") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) + 
  scale_fill_manual(values = c("#8DA0CB", "#B3B3B3", "#E5C494")) +
  facet_nested(~ Diet + Site, scales = "free_x", nest_line = TRUE) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        strip.background = element_blank(),
...
bp_shared_feed <- ggplot(shared_feed, aes(x = Segment, y = Abundance)) +
  geom_boxplot(fill = "#B3B3B3", width = 0.5) +
  geom_point() +
