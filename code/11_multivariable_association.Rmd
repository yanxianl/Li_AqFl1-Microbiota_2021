...
Based on the existing literature and the above exploratory data analysis, we include *foxp3* and *plin2* in the multivariable association analysis in addition to the main effects `Diet` and `Compartment`.
...
```{r}
# Fixed effects
fixef <- c("Diet", "Compartment", "foxp3", "plin2") 
...
fig7d <- sigAss_mcs %>%
  filter(metadata == "foxp3" & SampleOrigin != "Feed") %>%
...
```{r, fig.width=3, fig.height=4}
fig7e <- sigAss_mcs %>%
  filter(metadata == "plin2" & SampleOrigin != "Feed") %>%
