...
Based on the existing literature and the above exploratory data analysis, we include *foxp3*, *cd36* and *fabp2b* in the multivariable association analysis in addition to the main effects `Diet` and `Compartment`.
...
```{r}
# Fixed effects
fixef <- c("Diet", "Compartment", "cd3gd", "foxp3", "cd36", "fabp2b") 
...
fit_dgs_results <- filter(fit_dgs$results) %>%
  mutate(
    ...
    metadata = factor(metadata, levels = gsub("cd3gd", "cd3γδ", fixef))) 
...
names(scatter_dgs) <- gsub("cd3gd", "cd3γδ", fixef)[-c(1:2)]
```
...
Highlight the abundance of *Bacillus* and *Oceanobacillus* in REF and IM diet.

```{r, fig.width=5, fig.height=3.5}
fig6_diet <- filter(
  sigAss_dgs_diet,
  SampleOrigin == "Digesta" & feature %in% c("Bacillus", "Oceanobacillus")) %>%
  ggplot(aes(x = Diet, y = Abundance)) +
    geom_boxplot(width = 0.5, outlier.shape = NA) +
    geom_jitter(aes(color = Compartment), position = position_jitter(0.2))  +
    geom_point(
      data = filter(
        sigAss_dgs_diet, 
        SampleOrigin == "Feed" & feature %in% c("Bacillus", "Oceanobacillus")), 
      color = "#666666", size = 3) +
    geom_text(
...
Highlight the significant association between *Sphingobacterium* and *foxp3*.

```{r, fig.width=3, fig.height=4}
fig6_foxp3 <- sigAss_dgs %>%
  filter(
    metadata == "foxp3" & 
    SampleOrigin != "Feed" & 
    feature == "Sphingobacterium") %>%
  ggplot(aes(x = foxp3, y = log(Abundance))) +
    ...
    scale_y_continuous(expand = expansion(mult = c(0, 0.5))) +   
    labs(
      x = "Standardized expression of *foxp3*", 
      y = "Relative abundance (log scale)") +   
    scale_color_brewer(palette = "Dark2") 
...
Highlight the significant association between *Pediococcus* and *cd36*.

```{r, fig.width=3, fig.height=4}
fig6_cd36 <- sigAss_dgs %>%
  filter(
    metadata == "cd36" & 
    SampleOrigin != "Feed" & 
    feature == "Pediococcus") %>%
  ggplot(aes(x = foxp3, y = log(Abundance))) +
    ...
    scale_y_continuous(expand = expansion(mult = c(0, 0.5))) +   
    labs(
      x = "Standardized expression of *cd36*", 
      y = "Relative abundance (log scale)") +   
    scale_color_brewer(palette = "Dark2") 
...
names(scatter_mcs) <- gsub("cd3gd", "cd3γδ", fixef)[-c(1:2)]
```
