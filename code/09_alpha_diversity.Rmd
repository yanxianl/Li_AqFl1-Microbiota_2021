...
One of the fitted models is singular. Let's look at the estimated random effects.

```{r}
lapply(lme_dgs, function(x) summary(x)$varcor) 
```

The standard deviation of the random effect `FishID` is zero for Shannon's index. We update the model by removing the term `1|FishID`.

```{r, message=TRUE}
lme_dgs1 <- lapply(lme_dgs, function(x){
  if(isSingular(x$full_model) == TRUE) { # remove random effect FishID if the model is singular
    mixed(log1p(value) ~ Diet * Compartment + (1|Tank), 
          data = x$data, method = "KR")
  } else {
    x
  }
}
...
The updated model is no longer singular. Robust results should hold in both maximal and reduced model. Therefore, let us compare the pattern of significant and non-significant effects before and after removing the random effect `FishID`.

```{r}
left_join(
  nice(lme_dgs[["Shannon's index"]]), 
  nice(lme_dgs1[["Shannon's index"]]), 
  by = "Effect", suffix = c("_full", "_reduced"))
```

The pattern of significant and non-significant effect is basically the same for both models. Before proceeding to model diagnostics, let's look at model outputs.
...
lapply(
  ...
  {
    # extract titles 
    main <- ggdraw() + draw_label(names(lme_dgs1)[x], fontface='bold')
    
    # make residual diagnostic plots
    resid_panel <- resid_panel(lme_dgs1[[x]]$full_model, plots = "all", qqbands = TRUE)
...
intr_dgs <- lapply(lme_dgs1, function(x) {
...
cc_dgs <- mapply(function(x, y){
  ...
  if (y == TRUE){
    contrast(emm, method = "consec", simple = "each", combine = TRUE, adjust = "holm") %>%
    summary()
  }
},
  x = lme_dgs1, 
...
The standard deviation of random effect `FishID` is near or equals zero. We update the models by removing the term `1|FishID`.
...
Now we compare the pattern of significant and non-significant effects before and after removing the random effect `FishID` to make sure that results obtained are robust.
...
The pattern of significant and non-significant effect is basically the same for both models. Let's look at model outputs before proceeding to model diagnostics.
...
The residual plots of Shannon's index alert cautions. Everything else looks fine. We proceed to post-hoc tests.
...
alph_tm <- filter(alph, SampleOrigin %in% c("Water", "Mucosa")) %>%
  group_by(alph_indx, SampleOrigin, Diet, Compartment, Tank) %>%
...
t_test <- pivot_wider(alph_tm, names_from = SampleType, values_from = value) %>%
  ...
  select(alph_indx, Diet, comparison, p_raw, p_adj)
...
Gather *p*-values of main effects.
...
p_cc <- bind_rows(p_cc_dgs, p_cc_mcs) %>%
  ...
  mutate(
    ...
    y = c(0.82, 0.86, 0.96, 1.0, 0.90, 0.94, 1.04, 1.08,
          4.2, 4.4, 3.75, 4.0, 4.6, 4.8, 4.25, 4.5)) 
```

Make new alpha diversity plots.

```{r}
plist_fig.4 <- lapply(levels(alph$alph_indx), function(x) {
  # filter data
...
```{r}
plist_fig.s2 <- lapply(levels(alph_tm$alph_indx), function(x) {
  filter(alph_tm, alph_indx == x) %>%
  ggplot(aes(x = SampleType, y = value)) +
    geom_boxplot(aes(color = SampleType)) +
    geom_point(aes(color = SampleType)) +
    geom_signif(
      data = filter(p_t.test, alph_indx == x),
...
plot_grid(
  plist_fig.s2[[1]] + theme(axis.text.x = element_blank()),
  plist_fig.s2[[2]] + theme(axis.text.x = element_blank()),
  plist_fig.s2[[3]] + theme(plot.margin = margin(t = -0.5, unit = "cm")) + 
    labs(x = "Sample type"),
  plist_fig.s2[[4]] + theme(plot.margin = margin(t = -0.5, unit = "cm")) + 
    labs(x = "Sample type"),
  ncol = 2, align = "v") 

# export plot
ggsave(here("result/figure/Figure S2.tiff"), width = 8, height = 6,
       units = "in", dpi = 300, compression = "lzw")
```
