...
library(ggResidpanel) # Panels and Interactive Versions of Diagnostic Plots, CRAN v0.3.0 
...
alph <- map_dfc(alph, ~.x$data) %>% # merge alpha diversity indices
  ...
  rename(
    "Observed OTUs" = observed_features, 
  ...
  mutate(
    ...
    alph_indx = factor(
      alph_indx, 
      levels = c(
        "Observed OTUs", "Pielou's evenness", "Shannon's index", "Faith's PD"
        )
      )
    )
```
...
lme_dgs <- lapply(alph_dgs_spl, function(x) {
  mixed(
    log1p(value) ~ Diet * Compartment + (1|FishID) + (1|Tank), 
    data = x, 
    method = "KR"
    )
  }
...
lme_dgs1 <- lapply(lme_dgs, function(x){
  if(isSingular(x$full_model) == TRUE) { # remove random effect "FishID" if the model is singular
    mixed(
      log1p(value) ~ Diet * Compartment + (1|Tank), data = x$data, method = "KR"
      )
  } else {
    x
  }
...
The updated model is no longer singular. Robust results should hold in both maximal and reduced model. Let's compare the pattern of significant and non-significant effects before and after removing the random effect `FishID`.
...
lme_mcs1 <- lapply(alph_mcs_spl, function(x) {
  mixed(
    log1p(value) ~ Diet * Compartment + (1|Tank), data = x, method = "KR"
    )
  }
)
...
The updated models are no longer singular. Let's compare the pattern of significant and non-significant effects before and after removing the random effect `FishID`.
...
The residual plots of Shannon's index look OKish. Everything else is fine. We proceed to post-hoc tests.
...
cc_mcs <- mapply(function(x, y){
  ...
  if (y == TRUE){
    contrast(
      emm, method = "consec", simple = "each", combine = TRUE, adjust = "mvt"
      ) %>%
    summary()
  }
},
...
t_test <- alph_tm %>%
  ...
  mutate(
    t_test = map(
      data, 
      ~t.test(
        .x$Water, .x$value, paired = TRUE, alternative = "two.sided")
      ),
...
p_cc <- bind_rows(p_cc_dgs, p_cc_mcs) %>%
  ...
  mutate(
    ...
    y = c(0.82, 0.85, 0.96, 1.0, 0.9, 0.95, 1.05, 1.1,
          5.8, 6.0, 5.4, 5.6, 6.3, 6.6, 5.9, 6.2)
    ) 
```
...
For simplicity, we'll only show Shannon's index and Faith's PD in the Figure 4.
...
# export plot
ggsave(here("result/figure/Figure 4.tiff"), width = 7, height = 6,
...
bp_wt_mcs <- alph_tm %>%
  ...
  scale_color_manual(values = c("grey50", brewer.pal(2, "Paired"))) +
  labs(x = "Sample type") +
  theme_bw(base_size = 12) +
...
p_t.test <- ungroup(t_test) %>%
  ...
  y = c(120, 130, 0.92, 0.95, 5.3, 5.50, 40, 43))
